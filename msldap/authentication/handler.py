    ...
    def select(self):
        if self.login_credential.auth_method == LDAPAuthProtocol.SSPI:
            ...
            if self.login_credential.auth_method == LDAPAuthProtocol.MULTIPLEXOR:
                con_str = f"ws://{self.login_credential.settings['host'][0]}:{self.login_credential.settings['port'][0]}"
            else:
                con_str = f"wss://{self.login_credential.settings['host'][0]}:{self.login_credential.settings['port'][0]}"
