...
import ssl
from ldap3 import Server, Connection, Tls, ALL, NTLM, SIMPLE, BASE, ALL_ATTRIBUTES, LEVEL
...
class MSLDAPConnection:
    ...
    def connect(self):
        ...
        if self.login_credential.is_anonymous() == True:
            ...
            self._srv = Server(self.target_server.get_host(), use_ssl=self.target_server.is_ssl(), get_info=ALL, tls = tls)
