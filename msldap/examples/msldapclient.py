...
class MSLDAPClientConsole(aiocmd.PromptToolkitCmd):
    ...
    async def do_adinfo(self, show=True):
        ...
        try:
            if self.adinfo is None:
                ...
                self.domain_name = (
                    self.adinfo.distinguishedName.replace("DC", "")
                    .replace("=", "")
                    .replace(",", ".")
                )
            if show is True:
        ...
        try:
            ...
            try:
                ...
                with zipfile.ZipFile(
                    zip_filename, "a", compression=zipfile.ZIP_LZMA
                ) as dz:
            ...
            try:
                ...
                schema = {}
                with open(users_filename, "w", newline="", encoding="utf8") as f:
                    async for user, err in self.connection.get_all_schemaentry(
                        ["name", "schemaidguid"]
                    ):
                        if err is not None:
                            raise err
            ...
            try:
                dns_filename = "dns_%s.tsv" % datetime.datetime.now().strftime(
                    "%Y%m%d-%H%M%S"
                )
                ...
                with open(dns_filename, "w", newline="", encoding="utf8") as f:
                    async for (
                        ...
                        dnsrecod,
                        err,
                    ) in self.connection.dnsentries():
                        if err is not None:
                            raise err
                        dnsdataobj = dnsrecod.get_formatted()
                        line = "\t".join(
                            [zonedn, name, dnsrecod.Type.name, dnsdataobj.to_line()]
                        )
                        f.write(line + "\r\n")
                        pbar.update(1)
    ...
    async def do_cat(self, attributes="*", dn=""):
        ...
        async for entry, err in self.connection.pagedsearch(
            query="(distinguishedName=%s)" % dn, attributes=attributes, tree=dn
        ):
            if err is not None:
                raise err
    ...
    async def do_modify(self, dn, attribute, value):
        ...
        changes = {attribute: [("replace", value)]}
    ...
    async def do_groupmembers(self, dn, recursive=True):
        ...
        try:
            if recursive is not False or recursive != "0":
                recursive = True
            async for obj, err in self.connection.get_group_members(
    ...
    async def do_addallowedtoactonbehalfofotheridentity(
        self, target_dn, other_identity_sid
    ):
        ...
        try:
            ...
            async for entry, err in self.connection.pagedsearch(
                query, ["msDS-AllowedToActOnBehalfOfOtherIdentity", "objectSid"]
            ):
                if err is not None:
                    raise err
                ...
                if (
                    ...
                    print(
                        "Attribute does not exist on the current object, crafing new SD"
                    )
                    target_sd = SECURITY_DESCRIPTOR()
                    target_sd.Control = (
                        SE_SACL.SE_DACL_PRESENT | SE_SACL.SE_SELF_RELATIVE
                    )
        ...
        try:
            ...
            for ace in sd.Dacl.aces:
                if SID.wellknown_sid_lookup(ace.Sid.to_sddl()):
                    ...
                    domain, username, err = await self.connection.resolv_sid(
                        ace.Sid.to_sddl()
                    )
                    if username != "???":
    ...
    async def do_addenrollmentright(self, certtemplatename, user_dn):
        ...
        try:
            ...
            ace.Mask = (
                ADS_ACCESS_MASK.READ_PROP
                | ADS_ACCESS_MASK.WRITE_PROP
                | ADS_ACCESS_MASK.CONTROL_ACCESS
            )
                ...
                if memberships is not None:
                    for entry in memberships.Dacl.aces:
                        if entry.AceType == ACEType.ACCESS_ALLOWED_ACE_TYPE:
                            try:
                                user = await self.do_sidresolv(
                                    entry.Sid, to_print=False
                                )
    ...
    async def do_addcomputer(self, computername=None, password=None):
        ...
        try:
            computer, password, err = await self.connection.add_computer(
    ...
    async def do_pre2000(self):
        ...
        try:
            ...
            async for entry, err in self.connection.pagedsearch(
                query, attributes=["sAMAccountName"]
            ):
                if err is not None:
                    raise err
    ...
    if args.url is not None:
        ...
        if args.target is not None:
            ...
            if userinfo["logonserver"] is not None and len(userinfo["logonserver"]) > 0:
                ...
                raise Exception(
                    "Couldnt find logonserver! Are you connected to a domain?"
                )
...
"""
    ...
    parser.add_argument(
        "-v", "--verbose", action="count", default=0, help="Verbosity, can be stacked"
    )
    ...
    if platform.system() == "Windows":
        ...
        group2.add_argument(
            "--authtype", default="ntlm", help="Connection string in URL format."
        )
